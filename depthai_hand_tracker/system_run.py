...
from mediapipe_utils import HandRegion
import argparse
import serial
import random
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # Print xyz data
        for hand in hands:
            x = format(hand.xyz[0]/10, '.4')
            y = format(hand.xyz[1]/10, '.4')
            z = format(hand.xyz[2]/10, '.4')
            xyz = x + "|" + y + "|" + z + "|n"
            #print(xyz)
            ser.write(xyz.encode('ascii'))
            #print("X: %0.1f cm, Y: %0.1f cm, Z: %0.1f cm" % (x,y,z))
